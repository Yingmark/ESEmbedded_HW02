...
## 3. 結果與討論
 ...
 2. 根據 [Cortex-M4-Arm Developer](https://developer.arm.com/products/processors/cortex-m/cortex-m4)，由於 Cortex-M4 只支援 Thumb/ Thumb-2 指令，使用 `bl` 時，linker 自動把 pc 下一行指令位置並且設定 LSB 寫入 `lr` ，下一行指令  0x12 並且LSB 為 1 所以寫入 0x13 至 `lr` ，未來使用 `bx lr` 等指令時能確保是在 Thumb/ Thumb-2 指令下執行。
